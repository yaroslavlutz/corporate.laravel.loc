...
class Comment extends Model
{
    ...
    */
    public static function get_entries( $select=FALSE, $pagination=FALSE, $relationload=FALSE, $where=array() ) {
        ...
        if( !$pagination ) {  //Если пагинация не нужна
            ...
            else { $result_entries = self::select( $select )->get();  }  //Comments::get( $select );
        ...
        //Comment::get( array('id', 'parent_comment_id', 'text', 'name', 'email', 'site', 'user_id', 'article_id', 'created_at', 'updated_at') )->toArray(); //Comment::all()->toArray();
        //Comment::select('id', 'parent_comment_id', 'text', 'name', 'email', 'site', 'user_id', 'article_id', 'created_at', 'updated_at')->get()->toArray();
    }


    /** RELATIONSHIPS: */
    /* Указываем,что мы хотим использовать связь табл.`comments` c табл.`articles` (последняя табл.у нас ассоциирована с Моделью `App/Article.php`).
    ...
    */
    public function articles() {  //аналогично-противоп.метод прописан в Модели 'app/Comment.php'
    ...
    /* Указываем,что мы хотим использовать связь табл.`comments` c табл.`users` (последняя табл.у нас ассоциирована с Моделью `App/User.php`).
