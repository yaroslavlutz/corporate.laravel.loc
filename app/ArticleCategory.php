...
class ArticleCategory extends Model {
    ...
    */
    ...
    public static function get_entries( $select=FALSE, $pagination=FALSE, $relationload=FALSE, $where=array() ) {
        ...
        if( !$pagination ) {  //Если пагинация не нужна
            ...
            else { $result_entries = self::select( $select )->get();  }  //ArticleCategory::get( $select );
        ...
        //ArticleCategory::get( array('id', 'parent_cat_id', 'title', 'alias', 'created_at') )->toArray(); //ArticleCategory::all()->toArray();
        //ArticleCategory::select('id', 'parent_cat_id', 'title', 'alias', 'created_at')->get()->toArray();
    ...
    */
    ...
    /** RELATIONSHIPS: */
    /* Указываем,что мы хотим использовать связь табл.`articles_categories` c табл.`articles` (последняя табл.у нас ассоциирована с Моделью `Article.php`).
