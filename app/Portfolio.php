...
class Portfolio extends Model
{
    ...
    */
    ...
    protected $guarded = [
        //'id', 'alias', 'title', 'text', 'customer', 'images', 'portfolio_filter_alias', 'created_at', 'updated_at'
    ];
    ...
    */
    ...
    public static function get_entries( $select=FALSE, $pagination=FALSE, $relationload=FALSE, $where=array() ) {
        ...
        if( !$pagination ) {  //Если пагинация не нужна
            ...
            else { $result_entries = self::select( $select )->get();  }  //Portfolio::get( $select );
        ...
        //Portfolio::get( array('id', 'alias', 'title', 'text', 'customer', 'images', 'portfolio_filter_alias') )->toArray(); //Portfolio::all()->toArray();
        //Portfolio::select('id', 'alias', 'title', 'text', 'customer', 'images', 'portfolio_filter_alias')->get()->toArray();
    }


    /** RELATIONSHIPS: */
    /* Указываем,что мы хотим использовать связь табл.`portfolios` c табл.`portfolio_filters` (последняя табл.у нас ассоциирована с Моделью `App/PortfolioFilter.php`).
       Поле `portfolio_filter_alias` из табл.`portfolios` как внешний ключ(foreign-key) и связан с полем `alias` табл.`portfolio_filters`.
       Данная связь `1: ко многим` - 1 фильтр из табл.`portfolio_filters` может иметь много записей(портфолио) в табл.`portfolios`
    */
    public function PortfolioFilters() {  //аналогично-противоп.метод прописан в Модели `App/PortfolioFilter.php`
