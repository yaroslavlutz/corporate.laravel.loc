...
class PortfolioFilter extends Model
{
    ...
    */
    ...
    protected $guarded = [
        //'id', 'alias', 'title', 'created_at', 'updated_at'
    ];
    ...
    */
    ...
    public static function get_entries( $select=FALSE, $pagination=FALSE, $relationload=FALSE, $where=array() ) {
        ...
        if( !$pagination ) {  //Если пагинация не нужна
            ...
            else { $result_entries = self::select( $select )->get();  }  //PortfolioFilter::get( $select );
        ...
        //$result_portfolio_filters = PortfolioFilter::get( array('id', 'alias', 'title') )->toArray(); //PortfolioFilter::all()->toArray();
        //$result_portfolio_filters = PortfolioFilter::select('id', 'alias', 'title')->get()->toArray();
    }
    ...
    */
    ...
    /** RELATIONSHIPS: */
    /* Указываем,что мы хотим использовать связь табл.`portfolio_filters` c табл.`portfolios` (последняя табл.у нас ассоциирована с Моделью `App/Portfolio.php`).
    ...
    */
    public function portfolios() {  //аналогично-противоп.метод прописан в Модели 'App/Portfolio.php'
